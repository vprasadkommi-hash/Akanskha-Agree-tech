...
function OriginkitBaseGlitchCharReveal(props: any) {
    props = { ...COMPONENT_DEFAULTS, ...props }
    const { words = "Coming Soon", enterAnimation, hoverAnimation, color, font, tag } = props
    ...
    // ── Hover props ───────────────────────────────────────────────────────────
    const hoverType: string = hoverAnimation?.type ?? "wave"
    ...
    // Split into words while preserving exact whitespace
    ...
    const [, setPlacedChars] = useState<Record<number, number[]>>({})
    ...
    const detectLines = () => {
        ...
        if (allLines.length > 0) {
            ...
            setLineGroups([allWords.map((w) => w.globalWi)])
        }
    }
        ...
        >
            {paragraphs.map((_, pi) => {
                ...
                return (
                    ...
                    >
                        {/* Ghost elements for line measurement */}
                        <div
                            style={{
                                position: "absolute",
                                top: 0,
...
export default function GlitchCharReveal(props: Record<string, unknown>) {
  return <OriginkitBaseGlitchCharReveal {...COMPONENT_DEFAULTS} {...props} />;
}
